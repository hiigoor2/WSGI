...
class Imagem_upload {
    ...
    public function Salvar($caminho, $file) {
        $array_retorno = array();  // Retiramos acentos, espaços e hífens do nome da imagem
        $file['name'] = $this->tirarAcento(($file['name']));  // Atribuímos caminho e nome da imagem a uma variável apenas
        ...
        if (array_search($tipo, $this->tipos) === false) {
            $array_retorno["mensagem"] = "<font color='#F00'>Envie apenas imagens no formato jpeg, png ou gif!</font>";
        ...
        // Se a imagem temporária não for movida para onde a variável com caminho e nome indica, exibiremos uma mensagem de erro
            ...
            switch ($file['error']) {
                case 1:
                    $array_retorno["mensagem"] = "<font color='#F00'>O tamanho do arquivo é maior que o tamanho permitido.</font>";
                    break;
                case 2:
                    $array_retorno["mensagem"] = "<font color='#F00'>O tamanho do arquivo é maior que o tamanho permitido.</font>";
                    break;
                case 3:
                    $array_retorno["mensagem"] = "<font color='#F00'>O upload do arquivo foi feito parcialmente.</font>";
                case 4:
                    $array_retorno["mensagem"] = "<font color='#F00'>Não foi feito o upload de arquivo.</font>";
                    break;
            } // -> fim switch
        // Se a imagem temporária for movida
        } /* -> fim if */ else {// Pegamos sua largura e altura originais
            ...
            $array_retorno["mensagem"] = "<a href='" . $uploadfile . "'><font color='#070'>Upload realizado com sucesso!</font><a>";// Exibiremos uma mensagem de sucesso
        } // -> fim else
        $array_retorno["imagem"] = $uploadfile; //$array_retorno = array(); (No início da função salvar na classe UploadImagem) 
        return $array_retorno;// Retornamos a mensagem com o erro ou sucesso
    }
